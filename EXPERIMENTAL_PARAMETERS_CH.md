...
本文档提供本研究中所有实验参数的完整规格说明。参数选择旨在系统性地评估防御机制在从理想到挑战性的各种网络条件下的性能表现。实验脚本和工具在 **Python 3.9+** 环境中验证，可在macOS 14.x和Ubuntu 22.04上直接重现。
...
**注**：`attack_mode`在实验1-2中为`post`（后运行攻击），在实验3中为`inline`（实时混入攻击），以提供更严格的评估。
...
**预期结果**（定性）：
- 无防御：攻击成功率保持高位（所有丢包率）
- 滚动计数器：可用性随丢包率线性下降（在无乱序条件下，主要受信道影响）
- 滑动窗口：可用性下降趋势与滚动计数器相似（无乱序时）
- 挑战-响应：可用性受双向通信开销影响，但安全性最佳

*注：上述为设计阶段的定性预期，实际数值取决于具体实现和信道模拟细节，详见实验结果。*
...
**为什么使用实时混入攻击模式？**  
实时混入攻击（inline，在合法流量期间注入重放）比事后攻击更难防御。这为窗口大小权衡提供了更严格的评估。
...
**蒙特卡洛运行次数（n=200）**：
- 置信水平：95%（α = 0.05）
- 统计精度：200次运行提供充足的样本量，确保均值估计稳定（实测标准差约1.8%）
- 足以观察防御机制之间的统计显著性差异（差异>5%时可检测）
- 性能：200次运行约5.3秒（实测）
...
**重放攻击次数（n=100）**：
- 5:1的攻击-合法比例代表高压力测试场景
- 足够的样本量用于准确估计攻击成功率（100次样本）
- 选择理由：平衡评估严格性与计算效率
...
详细讨论：参阅 [`PRESENTATION_CH.md`](PRESENTATION_CH.md#6-技术实现细节)
