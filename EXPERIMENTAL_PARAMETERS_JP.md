...
本文書は、本研究で使用するすべての実験パラメータの完全な仕様を提供します。パラメータの選択は、理想的な条件から困難な条件まで、さまざまなネットワーク条件下で防御メカニズムを体系的に評価するように設計されています。スクリプトおよびツールは **Python 3.9+**（macOS 14.x / Ubuntu 22.04）で検証済みです。
...
| `attack_mode` | `post` | 事後攻撃スケジューリング（実験1-2）。実験3では `inline`（リアルタイム混入攻撃）に切り替えてより厳格な評価を実施 |
...
**なぜリアルタイム混入（inline）攻撃モードを使用するのか？**  
リアルタイム混入攻撃（inline、正規トラフィック中に注入されるリプレイ）は、事後攻撃よりも防御が困難です。これにより、ウィンドウサイズのトレードオフをより厳格に評価できます。
...
**予想される結果（定性的）**：
- ウィンドウサイズが大きくなるほど可用性は向上（小さいウィンドウは遅延パケットを拒否しやすい）
- 同時に攻撃成功率も増加（大きなウィンドウはリプレイ機会を増やす）
- 双方のバランスを取る最適範囲が存在（経験的には W=3-7）

*注: これらは設計段階の定性的期待値です。p_loss=15%, p_reorder=15%, リアルタイム混入（inline）攻撃における実測結果は PRESENTATION_JP.md 第8.4節を参照してください。*
...
詳細な議論：[`PRESENTATION_JP.md`](PRESENTATION_JP.md#6-技術実装詳細) を参照
