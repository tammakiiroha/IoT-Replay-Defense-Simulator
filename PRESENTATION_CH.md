...
| 机制 | 可用性 | 攻击成功率 | 定位 |
|------|--------|-----------|------|
| **Challenge** | ~90-100% | <0.1%-0.3% | ✅ 左上角：最佳安全性 |
| **Window (W=5)** | ~85-90% | 0.5-7.7% | ✅ 左上偏中：最佳平衡 |
| **Rolling** | ~76-90% | 0.1% | ⚠️ 左中：乱序时可用性下降 |
| **No Defense** | ~90% | 89.6% | ❌ 右侧：完全不设防 |

**注**：具体数值取决于网络条件（丢包率、乱序率）和攻击模式
...
**实验条件**：p_loss=15%, p_reorder=15%, 实时混入攻击（inline）。README中的窗口扫描表格使用相同数据源和条件。
...
| 维度 | 实现内容 |
|------|---------|
| **测试覆盖** | 94个测试用例，覆盖核心防御逻辑、信道模拟、统计计算 |
...
**Q5：如果攻击者针对窗口漏洞怎么办？**
> A：窗口机制的攻击成功率与网络条件和攻击模式相关。实验3（p_loss=15%, p_reorder=15%, 实时混入攻击）显示W=5时攻击率约7.7%；在post攻击模式下（实验1/2），攻击率可低至0.5-1.8%。虽然理论上存在重放窗口，但实验结果表明：相比rolling机制在高乱序下13.5%的可用性下降，window机制的安全性代价是可接受的。实际部署需根据威胁模型权衡。
