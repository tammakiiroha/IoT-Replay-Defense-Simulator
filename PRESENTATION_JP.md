...
8. [主要な実験結果](#8-主要な実験結果)
9. [プロジェクト品質保証](#9-プロジェクト品質保証)
10. [制約と今後の課題](#10-制約と今後の課題) ⭐ 新規追加
11. [専門用語集](#11-専門用語集)
12. [デモンストレーション](#12-デモンストレーション)
...
| メカニズム | ユーザビリティ | 攻撃率 | ポジション |
|-----------|--------------|--------|-----------|
| **Challenge** | ~90-100% | <0.1%-0.3% | ✅ 左上：最高セキュリティ |
| **Window (W=5)** | ~85-90% | 0.5-7.7% | ✅ 左上～中央：最適バランス |
| **Rolling** | ~76-90% | 0.1% | ⚠️ 左中央：並び替え時に利便性低下 |
| **No Defense** | ~90% | 89.6% | ❌ 右側：完全に無防御 |

**注記**：具体的な数値はネットワーク条件（パケット損失、並び替え）と攻撃モードに依存します
...
**実験条件**：p_loss=15%, p_reorder=15%, リアルタイム混入攻撃（inline）。READMEのウィンドウスイープ表も同一データソースと条件を使用。
...
#### 10.1.1 簡略化されたチャネルモデル
**現状**
- i.i.d（独立同分布）のパケット損失モデル
- 簡易的な遅延キューによる順序入れ替えのみを考慮

**制約**
- バースト損失（連続的に発生する損失）を再現していない
- 順序入れ替えが独立事象であり、ルーティング揺らぎや相関ジッタを表現できない
- 遅延分布が単純で、長い尾を持つレイテンシ特性を表現できない

**影響**
- 「ローリングは順序入れ替えに弱い」「ウィンドウは強い」といった定性的傾向は堅牢だが、実環境での絶対値は異なる可能性がある
- すべての防御手法が同じモデルを共有しているため、相対比較は有効

**今後の改善案**
- Gilbert-Elliott 型の2状態バースト損失モデルの導入
- Markov 過程による相関順序入れ替えモデル
- 実測トレースを読み込む trace-driven シミュレーション

---

#### 10.1.2 攻撃モデルの網羅範囲
**現状**
- リプレイ攻撃（事後攻撃とリアルタイム混入攻撃）のみを評価

**未カバーの攻撃**
1. **リレー攻撃**：暗号解読なしで距離を延長するリアルタイム転送  
2. **信号操作**：ジャミング、電力解析、タイミング攻撃など  
3. **プロトコル層攻撃**：セッションハイジャック、ダウングレード攻撃、鍵漏えい

**研究スコープ**
- 低消費電力無線で最も一般的な脅威がリプレイ攻撃であるため、本研究ではそこに集中
- その他の攻撃は異なる脅威モデルと対策を必要とする

**今後の拡張**
- リレー攻撃シミュレーションと距離バウンディング手法の評価
- リプレイと物理層攻撃の複合シナリオ
- 攻撃者モデルを段階的に拡張し、耐性を検証

---

#### 10.1.3 暗号実装
**現状**
- HMAC-SHA256 を 32bit（8桁の16進）に切り詰め、事前共有鍵(PSK)を仮定

**制約**
- 32bit MAC 空間（約 4.3 億）は高セキュリティ用途には不足の可能性
- 鍵更新や鍵配布をシミュレートしていない

**補足**
- すべての防御手法が同じ MAC 長を使用するため、相対的な優劣には影響しない
- 6.4節で述べた通り、実運用では 64–128bit への切り替えを推奨

**今後の改善案**
- 実験ごとに MAC 長を設定できるようにする
- 鍵ローテーションや鍵更新手順の導入
- AES-GCM や ChaCha20-Poly1305 などの AEAD 方式への対応

---

#### 10.1.4 物理層検証の不足
**現状**
- 完全にソフトウェア上でのシミュレーション（固定シード、パラメータスイープ）

**不足している要素**
- RF ハードウェア（ESP32 / nRF52 / CC2652 等）での実験
- レイテンシ、ジッタ、電力消費などの実測データ
- 環境依存要素（マルチパス、外乱源）の影響評価
...
**研究位置づけ**
- 理論解析と実機評価の中間にある「パラメトリックな評価基盤」
- 絶対性能の予測ではなく、設計指針を提供することが目的
...
#### 10.2.1 短期（6〜12か月）
**1. 物理実験による検証**
- ハードウェア: ESP32（Wi-Fi）、nRF52840（BLE）、CC2652（Zigbee）
- 3つのコア実験を実機チャネルで再現
- シミュレーションとの乖離を分析し、改善ポイントを抽出

**2. チャネルモデルの高度化**
- Gilbert-Elliott 型バースト損失モデル（IEEE 802.15.4 文献を参照）
- Markov 連鎖による相関順序入れ替えモデル
- PCAP などの実トレースを読み込む trace-driven シミュレーション
...
class RelayAttacker:
    ...
    def relay(self, frame):
        return frame  # そのまま受信側へ転送
```
- チャレンジレスポンスのリレー攻撃耐性を評価
- 距離バウンディング手法や時刻制約の検証

**4. マルチプロトコル対応**
- TLS 1.3 / DTLS の性能評価（制約付き IoT への適用）
- MQTT-SN セキュリティ拡張
- CoAP + DTLS（ブロック転送、Confirmable メッセージ）での実装

---

#### 10.2.3 長期（2〜5年）
**5. ハードウェア最適化とアクセラレーション**
- FPGA によるスライディングウィンドウのビットマップ実装
- マイコン上でのメモリ・電力最適化
- Cortex-M4/M7 でのリアルタイムベンチマーク

**6. 標準化コミュニティへの還元**
- IETF IoT WG へのデータセット提供
- Best Current Practice (BCP) ドキュメント作成
- IEEE INFOCOM / ACM SenSys 等でのチュートリアル・ワークショップ

---

### 10.3 コントリビューションのお願い
- **実験データ**: 実機での再現結果（JSON）を共有
- **コード**: 新しい防御手法、チャネルモデル、最適化
- **ドキュメント**: 翻訳、チュートリアル、解説記事
- **研究連携**: 共同論文、データセット交換、再現実験
...
### 10.4 謝辞
- チャネル範囲設計は IEEE 802.15.4 / BLE / LoRaWAN の信頼性研究から着想
- Python エコシステム（numpy, matplotlib, pytest）に感謝
- GitHub コミュニティおよびオープンソース貢献者に感謝
...
> A: windowメカニズムの攻撃成功率はネットワーク条件と攻撃モードに依存します。実験3（p_loss=15%, p_reorder=15%, リアルタイム混入攻撃）ではW=5で攻撃率約7.7%；post攻撃モード（実験1/2）では攻撃率は0.5-1.8%に低下します。理論上リプレイウィンドウは存在しますが、実験結果によると：高乱序下でのrollingメカニズムの13.5%の可用性低下と比較すると、windowメカニズムのセキュリティコストは許容範囲内です。実際の展開では脅威モデルに基づいて判断が必要です。
