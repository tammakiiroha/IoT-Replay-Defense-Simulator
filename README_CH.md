...
本仓库是我关于2.4 GHz无线控制/物联网系统重放攻击防御的工学学士毕业论文的实验和仿真工具包。

- 论文主题（概念性）：*2.4 GHz无线控制系统的重放攻击防御机制*
- 本仓库的角色：
  - 蒙特卡洛模拟器和实验驱动程序
  - 参数集和JSON结果文件
  - 论文和幻灯片中使用的图表
- 不包含的内容：
  > 论文全文（PDF/LaTeX源文件）**未在本仓库公开**。
  > 本项目专注于支持论文的代码、实验和技术文档。

如果您是审查员或潜在导师，本仓库展示了实验是如何设计和重现的。
...
尽管实验3显示W=3的综合得分略高于W=5（78.6 vs 77.7），本项目仍选择W=5作为默认值，理由如下：

| 考量因素 | W=3 | W=5 | 说明 |
|---------|-----|-----|------|
| **综合得分** | 78.6 | 77.7 | 差异仅0.9，统计上不显著 |
| **乱序容忍度** | 最多容纳3包乱序 | 最多容纳5包乱序 | W=5对网络抖动更鲁棒 |
| **安全裕度** | 6.5%攻击率 | 7.7%攻击率 | 差异1.2%，均在可接受范围 |
| **适用范围** | 低延迟网络更优 | 通用性更强 | W=5在更广泛条件下表现稳定 |

**核心考量**：
1. **保守设计原则**：W=5提供更大的乱序容忍空间，在网络条件比实验设定更差时（如突发延迟）不会突然失效
2. **实验条件的特殊性**：实验3的条件（p_loss=15%, p_reorder=15%）是特定组合，W=3的"最优"仅针对该条件成立
3. **工程实践**：实际部署中网络条件变化较大，选择稍大的窗口可以提供安全裕度

> 💡 **建议**：如果目标环境的网络特性已知且相对稳定，可以使用W=3以获得略优的综合性能；如果网络条件不确定或变化较大，W=5是更稳妥的选择。
