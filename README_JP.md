...
このリポジトリは、2.4 GHz無線制御/IoTシステムにおけるリプレイ攻撃防御に関する私の工学士卒業論文の実験・シミュレーションツールキットです。

- 論文テーマ（概念的）: *2.4 GHz無線制御システムにおけるリプレイ攻撃防御メカニズム*
- 本リポジトリの役割:
  - モンテカルロシミュレータと実験ドライバー
  - パラメータセットとJSON結果ファイル
  - 論文およびスライドで使用する図表
- 含まれないもの:
  > 論文全文（PDF/LaTeXソース）は本リポジトリでは**公開していません**。
  > このプロジェクトは、論文を支援するコード、実験、技術ドキュメントに焦点を当てています。

審査員または指導教員候補の方へ：このリポジトリは実験がどのように設計され、再現されたかを示しています。
...
**なぜデフォルト値がW=3ではなくW=5なのか？**

実験3ではW=3の総合スコアがW=5より若干高い（78.6 vs 77.7）にもかかわらず、本プロジェクトではW=5をデフォルト値として採用しています。その理由：

| 考慮要素 | W=3 | W=5 | 説明 |
|---------|-----|-----|------|
| **総合スコア** | 78.6 | 77.7 | 差はわずか0.9、統計的に有意ではない |
| **並び替え許容度** | 最大3パケット | 最大5パケット | W=5はネットワークジッタに対してより堅牢 |
| **セキュリティマージン** | 6.5%攻撃率 | 7.7%攻撃率 | 差は1.2%、両方とも許容範囲内 |
| **適用範囲** | 低遅延ネットワークで優位 | より汎用的 | W=5はより広範な条件で安定 |

**核心的な考慮事項**：
1. **保守的設計原則**：W=5はより大きな並び替え許容空間を提供し、実験設定より悪いネットワーク条件（バースト遅延など）でも突然失敗しない
2. **実験条件の特殊性**：実験3の条件（p_loss=15%, p_reorder=15%）は特定の組み合わせであり、W=3の「最適」はその条件のみに当てはまる
3. **エンジニアリング実践**：実際の展開ではネットワーク条件の変動が大きく、やや大きなウィンドウを選択することで安全マージンを確保

> 💡 **推奨事項**：ターゲット環境のネットワーク特性が既知で比較的安定している場合、W=3を使用して若干優れた総合性能を得ることができます。ネットワーク条件が不確実または変動が大きい場合、W=5がより安全な選択です。
...
### ウィンドウスイープ（ストレステスト: p_loss=0.15, p_reorder=0.15, inline攻撃）

_中程度のネットワークストレス下でのユーザビリティとセキュリティのトレードオフを比較。_
...
*注: 本表のデータは `results/window_sweep.json` からのもので、実験3および図表 `window_tradeoff.png` の条件と一致しています。*
