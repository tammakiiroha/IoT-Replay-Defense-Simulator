#!/usr/bin/env python3
"""Generate ASR-vs-loss benchmark figure."""
...
def main() -> int:
    ...
    spec = SweepSpec(
        ...
        simulation=SimulationSpec(
            modes=["no_def", "rolling", "window", "challenge", "hsw_cr"],
            runs=40,
            seed=7,
            window_size=8,
            num_legit=20,
    ...
    for mode in modes:
        ...
        ys = [point.result.avg_attack_rate for point in rows]
        low = [max(0.0, y - point.result.asr_ci_low) for y, point in zip(ys, rows)]
        high = [max(0.0, point.result.asr_ci_high - y) for y, point in zip(ys, rows)]
        ax.errorbar(xs, ys, yerr=[low, high], marker="o", linewidth=1.8, label=mode)

    ax.set_title("Attack success rate under packet loss")
    ax.set_xlabel("Packet loss probability")
    ax.set_ylabel("ASR")
    ax.set_ylim(-0.03, 1.03)
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    output = Path("docs/figures/asr_vs_loss.png")
